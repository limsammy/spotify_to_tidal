...
def main():
    ...
    parser.add_argument(
        "--remove-tracks",
        help="remove matching tracks from the Spotify playlist given by --uri "
        "before syncing, e.g. 'album:<spotify_album_id>' or "
    ...
    if args.remove_tracks:
        ...
        _sync.remove_tracks_from_spotify_playlist(
            spotify_session, args.uri, args.remove_tracks
        )
    if args.uri:
        ...
        # and favorites unless explicitly disabled
        _sync.sync_playlists_wrapper(
            spotify_session,
            tidal_session,
