...
class MatchFailureDatabase:
    ...
    def cache_match_failure(self, track_id: str):
        ...
        with self.engine.connect() as connection, connection.begin():
            # Either update the next_retry time if track_id already exists,
    ...
    def has_match_failure(self, track_id: str) -> bool:
        """checks if there was a recent failed match search for the given track_id"""
...
class TrackMatchCache:
    ...
    data: dict[str, int] = {}  # noqa: RUF012 -- intentional shared singleton state
...
class ManualMatchDatabase:
    """
    sqlite mapping of spotify track id -> tidal track id,
    persisting manually confirmed matches between runs
    """
    ...
    def insert(self, spotify_id: str, tidal_id: int):
        with self.engine.connect() as connection, connection.begin():
            connection.execute(
                delete(self.matches).where(self.matches.c.spotify_id == spotify_id)
            )
            connection.execute(
                insert(self.matches), {"spotify_id": spotify_id, "tidal_id": tidal_id}
            )
