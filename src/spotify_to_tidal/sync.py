...
from .cache import failure_cache, manual_match_cache, track_match_cache
...
def simple(input_string: str) -> str:
    # only take the first part of a string before any hyphens or brackets
    # to account for different versions
    return (
...
def name_match(tidal_track, spotify_track) -> bool:
    ...
    # the simplified version of the Spotify track name must be
    # a substring of the Tidal track name
...
def artist_match(tidal: tidalapi.Track | tidalapi.Album, spotify) -> bool:
    ...
    def get_tidal_artists(
        ...
        result: list[str] = []
        for artist in tidal.artists:  # ty: ignore[not-iterable]
            artist_name = normalize(artist.name) if do_normalize else artist.name
            result.extend(split_artist_name(artist_name))  # ty: ignore[invalid-argument-type]
            ...
            for album in album_result["albums"]:
                if album.num_tracks >= spotify_track[
                    ...
                    if len(album_tracks) < spotify_track["track_number"]:
                        # incorrect metadata :(
                        assert len(album_tracks) != album.num_tracks
                        continue
...
async def repeat_on_request_error(function, *args, remaining=5, **kwargs):
    ...
    try:
        ...
        if remaining:
            print(f"{e!s} occurred, retrying {remaining} times")
        else:
            print(f"{e!s} could not be recovered")
...
async def get_tracks_from_spotify_playlist(
    ...
    def _get_tracks_from_spotify_playlist(offset: int, playlist_id: str):
        fields = (
            "next,total,limit,items(track(name,album(name,artists),artists,"
            "track_number,duration_ms,id,external_ids(isrc))),type"
        )
    ...
    def track_filter(item):
        ...
        return item.get("type", "track") == "track"
...
def populate_track_match_cache(
    ...
    """Populate the track match cache with all the existing tracks in
    ...
    # then populate from the subset of Spotify tracks that didn't match
    # (to account for many-to-one style mappings)
...
def get_tracks_for_new_tidal_playlist(
    ...
    """gets list of corresponding tidal track ids for each spotify track,
    ignoring duplicates"""
    output = []
    seen_tracks = set()
    ...
    for spotify_track in spotify_tracks:
        ...
        if tidal_id:
            if tidal_id in seen_tracks:
                ...
                print(
                    f'Duplicate found: Track "{track_name}" by {artist_names} '
                    "will be ignored"
                )
            else:
...
async def search_new_tracks_on_tidal(
    ...
    """Generic function for searching for each item in a list of Spotify tracks
    which have not already been seen and adding them to the cache"""
    ...
    task_description = (
        f"Searching Tidal for {len(tracks_to_search)}/{len(spotify_tracks)} "
        f"tracks in Spotify playlist '{playlist_name}'"
    )
    ...
    for idx, spotify_track in enumerate(tracks_to_search):
        if search_results[idx]:
            ...
            song404.append(
                f"{spotify_track['id']}: "
                f"{','.join([a['name'] for a in spotify_track['artists']])}"
...
def remove_tracks_from_spotify_playlist(
    spotify_session: spotipy.Spotify, playlist_id: str, query: str
):
    """remove tracks matching 'album:<spotify_album_id>' or
    'track:<spotify_track_id>' from the Spotify playlist"""
    ...
    if kind == "album" and value:
        ...
        sys.exit(
            "Unsupported --remove-tracks query; use 'album:<spotify_album_id>'"
            " or 'track:<spotify_track_id>'"
        )

    playlist_tracks = asyncio.run(
        get_tracks_from_spotify_playlist(
            spotify_session, spotify_session.playlist(playlist_id)
        )
    )
    ...
    if not to_remove:
        ...
        return
    print("Tracks to remove from Spotify playlist:")
    for t in to_remove:
    ...
    if input(f"Remove these {len(to_remove)} track(s)? [y/N]: ").strip().lower() != "y":
        ...
        return
    spotify_session.playlist_remove_all_occurrences_of_items(
        playlist_id, [t["id"] for t in to_remove]
    )
    print("Removed from Spotify; the following sync will remove them from Tidal")


def interactively_match_tracks(
    tidal_session: tidalapi.Session, spotify_tracks: Sequence[t_spotify.SpotifyTrack]
):
    """prompt the user to manually pick a Tidal match for any track
    the automatic search failed on"""
    ...
    if not unmatched or not sys.stdin.isatty():
        return
    print(
        f"\n{len(unmatched)} track(s) had no automatic match. "
    ...
    for spotify_track in unmatched:
        ...
        candidates = [
            t
            for t in tidal_session.search(query, models=[tidalapi.media.Track])[
                "tracks"
            ]
            if t.available
        ][:5]
        if not candidates:
            candidates = [
                t
                for t in tidal_session.search(
                    simple(spotify_track["name"]), models=[tidalapi.media.Track]
                )["tracks"]
                if t.available
            ][:5]
        print(
            f"\nSpotify: {artists} - {spotify_track['name']} "
            f"({spotify_track['duration_ms'] // 1000}s)"
        )
        if not candidates:
            print("  No candidates found on Tidal, skipping")
            continue
        for i, c in enumerate(candidates, 1):
            c_artists = ", ".join(a.name for a in c.artists)  # ty: ignore[no-matching-overload,not-iterable]
            version = f" ({c.version})" if c.version else ""
            print(
                f"  {i}. {c_artists} - {c.name}{version} "
                f"[{c.album.name}] ({c.duration}s)"  # ty: ignore[unresolved-attribute]
            )
        while True:
            choice = (
                input(f"Pick a match [1-{len(candidates)}] or s(kip): ").strip().lower()
            )
            if choice in ("s", "skip", ""):
                break
...
async def sync_playlist(
    ...
    # Get the tracks from both Spotify and Tidal,
    # creating a new Tidal playlist if necessary
    ...
    if tidal_playlist:
        ...
        tidal_playlist = tidal_session.user.create_playlist(  # ty: ignore[unresolved-attribute]
    ...
    if new_tidal_track_ids == old_tidal_track_ids:
        ...
        add_multiple_tracks_to_playlist(
            tidal_playlist,  # ty: ignore[invalid-argument-type]
            new_tidal_track_ids[len(old_tidal_track_ids) :],
        )
    else:
        # Erase old playlist and add new tracks from scratch if any reordering occured
        clear_tidal_playlist(tidal_playlist)  # ty: ignore[invalid-argument-type]
        add_multiple_tracks_to_playlist(tidal_playlist, new_tidal_track_ids)  # ty: ignore[invalid-argument-type]
...
async def sync_favorites(
    ...
    old_tidal_tracks = await get_all_favorites(
        tidal_session.user.favorites,  # ty: ignore[unresolved-attribute]
        order="DATE",
    )
    populate_track_match_cache(spotify_tracks, old_tidal_tracks)  # ty: ignore[invalid-argument-type]
    await search_new_tracks_on_tidal(tidal_session, spotify_tracks, "Favorites", config)  # ty: ignore[invalid-argument-type]
    new_tidal_favorite_ids = get_new_tidal_favorites()
    if new_tidal_favorite_ids:
        for tidal_id in tqdm(
            new_tidal_favorite_ids,
            desc="Adding new tracks to Tidal favorites",
            dynamic_ncols=True,
        ):
            tidal_session.user.favorites.add_track(tidal_id)  # ty: ignore[unresolved-attribute]
    else:
...
def get_tidal_playlists_wrapper(
    ...
    tidal_playlists = asyncio.run(get_all_playlists(tidal_session.user))  # ty: ignore[invalid-argument-type]
    return {playlist.name: playlist for playlist in tidal_playlists}  # ty: ignore[invalid-return-type]
...
def pick_tidal_playlist_for_spotify_playlist(
    ...
    if spotify_playlist["name"] in tidal_playlists:
        # if there's an existing tidal playlist with the name
        # of the current playlist then use that
